...
description: DBからクリップ構成を読み込み、Sora 2のInitial生成+Extend×6用のプロンプトを生成・ファイル保存・DB保存する。Use when Step10（Sora2プロンプト生成）を実行する時。「step10」「Sora 2プロンプト」「sora prompts」などのキーワードが出たら必ずこのスキルを使うこと。
---
...
Step8で生成された7クリップ構成（7ビート × 10秒 = 70秒）を元に、Sora 2に直接コピペできるプロンプトを生成する。

- **clip1.txt**: Initial生成用（完全なプロンプト）
- **clip2.txt〜clip7.txt**: Extend用（前クリップから延長する指示）
...
DBパス: `~/openclaw_state/short-movie.db`（SQLite）
```bash
sqlite3 ~/openclaw_state/short-movie.db "SELECT ..."
```

### 2. プロンプト生成

各クリップについて、以下のテンプレートに従いSora 2用プロンプトを**英語で**生成する。

### 3. ファイル保存

生成したプロンプトを以下のファイルに保存する:
```
~/openclaw_state/workspace/sora-prompts/ep1/clip1.txt
~/openclaw_state/workspace/sora-prompts/ep1/clip2.txt
~/openclaw_state/workspace/sora-prompts/ep1/clip3.txt
~/openclaw_state/workspace/sora-prompts/ep1/clip4.txt
~/openclaw_state/workspace/sora-prompts/ep1/clip5.txt
~/openclaw_state/workspace/sora-prompts/ep1/clip6.txt
~/openclaw_state/workspace/sora-prompts/ep1/clip7.txt
```
