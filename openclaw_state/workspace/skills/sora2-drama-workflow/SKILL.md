...
8. **Step8**:  `step8-generate-episodes`          — エピソード構造 + Sora2プロンプト一括生成（**人間レビュー必須**）
9. **Step9**:  `step9-insert-videos`              — generated_videos INSERT
10. ~~**Step10b**~~                               — **[Step8に統合済み・スキップ]**
...
- Step8はPhase A（エピソード構造）の人間承認後にPhase B（Sora2プロンプト生成）を連続実行する。
- Step8完了後、Sora 2が生成した4クリップを `ep{N}_clip{1-4}.mp4` で保存させる。
- **Step10bはStep8に統合済みのため呼び出さない**。
...
|------|-----------------------------------|-------------------------------|
| Step8 | エピソード構造のみ生成 | **エピソード構造 + Sora2プロンプト一括生成** |
| Step10 | `step10-prepare-scene-prompts` | **廃止**（Step8に統合） |
