...
Sora 2専用の完結したワークフロー。Step1〜7はNanoBananaワークフロー（`short-drama-workflow`）と共通、Step8以降がSora 2固有。

0. **【対象シリーズ選択】**（既存シリーズを続行する場合）
   - `generation_jobs` から作業対象のジョブ一覧を表示する:
     ...
     ORDER BY id DESC;
     ```
   - 例: id=2「5,000円の沈黙」(episodes_ready) / id=3「家計簿にない出費」(episodes_ready)
   - **人間が job_id を確認・選択する**
   - 以降の全ステップで選択した `job_id` を使用する
   - 新規シリーズ作成（Step1〜5）の場合はこの選択ステップは不要（新規 job_id が自動付与される）
...
8. **Step8**:  `step8-generate-episodes`          — エピソード構造生成・承認（**人間レビュー必須**）
9. **Step9**:  `step9-insert-videos`              — generated_videos INSERT
10. **Step10**: `step10-prepare-sora2-prompts`    — 各話を4クリップに分割・Sora2プロンプト生成（**人間レビュー必須**）
...
- **全ステップは選択した job_id に紐づくデータのみを参照すること**。複数シリーズが存在する場合でも、他のシリーズのデータを誤って参照しない。
- Step6（人間レビュー）承認なしで Step7 以降へ進まない。
- Step8はエピソード構造生成のみ行う。人間承認後にStep9（DB INSERT）へ渡す。
- Step9完了（`episodes_ready` 状態）後、Step10でDBからエピソードデータを読み込んでSora2プロンプトを生成する。
- Step10完了後、Sora 2が生成した4クリップを `ep{N}_clip{1-4}.mp4` で保存させる。
...
|------|-----------------------------------|-------------------------------|
| Step8 | エピソード構造のみ生成 | **エピソード構造生成・承認** |
| Step10 | `step10-prepare-scene-prompts` | **`step10-prepare-sora2-prompts`（4クリップSora2プロンプト生成）** |
