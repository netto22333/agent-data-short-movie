...
NanoBananaで毎シーン同じ見た目を再現するため、外見を固定の英語描写で定義します。

各キャラクター:
- name: キャラクター名（日本語）
- role: 役割（例: 主人公, 同僚, 上司）
- appearance: 外見の固定描写（英語・NanoBanana用。髪色/髪型/目の色/服装スタイルを含む）
...
各ロケーション:
- name: 場所名（日本語）
- appearance: 場所の固定描写（英語・NanoBanana用。部屋の構造/照明/背景要素を含む）
...
- image_prompt: NanoBanana用プロンプト（英語）
  ※ 必ず以下を順番に埋め込む:
...
- video_motion_prompt: 動き指示（英語・NanoBanana動画生成向け・カメラムーブや動作）
