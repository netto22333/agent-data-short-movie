...
description: 承認済み台本から1話完結の7クリップ構成（7ビート × 10秒）を自動生成し、セリフ・映像演出を具体化する。Use when Step8（クリップ構成生成）を実行する時。
---
...
3. Claude Code が承認済み台本から7クリップの詳細構成を自動生成し、そのまま採用する。
...
- 生成したクリップ構成をそのまま採用し、次ステップ（step9-insert-videos）へ渡す。
